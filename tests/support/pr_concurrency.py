...
Workflows are read through :func:`tests.support.workflows.load_workflow`,
which parses with ``yaml.BaseLoader``: every scalar arrives as a string,
so a literal ``cancel-in-progress: true`` reaches the reader as
``"true"`` and is rejected by the same branch that rejects the boolean.
"""
