//! Compile-fail guards for the `sha2` 0.11 migration.
//!
//! `sha2` 0.11 made a source-breaking change that this crate had to work
//! around: `Sha256::finalize()` / `Sha256::digest()` now return
//! `hybrid_array::Array<u8, _>`, which does not implement
//! [`core::fmt::LowerHex`], so `format!("{:x}", digest)` no longer compiles.
//! Digests are therefore rendered through the crate's `to_lower_hex` helper.
//!
//! This `trybuild` case pins that break: if a future change reintroduces the
//! pre-0.11 pattern — for example by downgrading `sha2` back to 0.10 — the
//! fixture starts compiling and this test fails, flagging the regression.

#[test]
fn sha2_0_11_lowerhex_formatting_fails_to_compile() {
    let cases = trybuild::TestCases::new();
    cases.compile_fail("tests/ui/fail/*.rs");
}
