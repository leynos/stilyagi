...
mod tests {
    ...
    proptest! {
        #[test]
        fn source_and_synthetic_segments_reconstruct_region_text(
            prefix in "[a-z]{0,12}",
            suffix in "[a-z]{0,12}",
        ) {
            let source_segment =
                IrSegment::source(0, prefix.clone(), SourceSpan::new(0, prefix.len()), "n1");
            let synthetic_segment =
                IrSegment::synthetic(prefix.len(), " ", "softbreak_space");
            let suffix_start = prefix.len() + 1;
            let suffix_segment = IrSegment::source(
                suffix_start,
                suffix.clone(),
                SourceSpan::new(20, 20 + suffix.len()),
                "n1",
            );
            let region = IrRegion {
                id: "r1".to_owned(),
                ...
                segments: vec![source_segment, synthetic_segment, suffix_segment],
