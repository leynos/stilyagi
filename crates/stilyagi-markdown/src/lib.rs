...
use stilyagi_ir::{
    DocumentMetadata, IrBuildContext, IrDocument, IrNode, IrRegion, IrSegment, IrTree, NodeFlags,
    ProducerMetadata, SegmentOrigin, SourceIdentity, SourceSpan,
};
...
pub fn markdown_ir_document(source: &str, identity: SourceIdentity) -> Result<IrDocument, Message> {
    ...
    markdown_ir_document_with_context(
        source,
        context,
        &MarkdownDiagnosticContext {
            phase: "parse",
...
fn markdown_ir_document_with_context(
    ...
    diagnostic_context: &MarkdownDiagnosticContext<'_>,
) -> Result<IrDocument, Message> {
    let ast = parse_markdown_ast_with_context(source, diagnostic_context, |value| {
