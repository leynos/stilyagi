...
- That payload is deliberately limited to `syntax` plus
  `regions[{kind, text}]`. It does not yet expose `line_index`,
  `segments`, owner metadata, or canonical IR JSON as part of the in-process
  bridge.
